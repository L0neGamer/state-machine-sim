...
instance Show a => Show (Tape a) where
  show Tape {..} = "Tape " ++ show (take 20 right) ++ (' ' : show (take 20 left)) ++ ' ' : displayCursor
    where
      displayCursor
        | cursor < 0 = '(' : show cursor ++ ")"
...
data TuringMachine a = TuringMac
  { states :: States,
    language :: Language a,
    blank :: a,
    startState :: State,
...
constructTuringMachine states language blank startState acceptStates transitions
  ...
  where
    (transitionStates, transitionLanguage') = getStatesAndLang (map (\(a, b, (c, _, _)) -> (a, b, c)) transitions)
...
instance RunningStateMachine RunningTM where
  ...
  step RunTM {..}
    ...
    | otherwise = runningTM
    where
      (nextState, toWrite, dir) = stepTuringMachine currentState (getVal tape) turingMachine
...
busyBeaverCheck (ones, steps, tm) = (ones - tapeSum, steps - timeSpent, runMachine)
  where
    runMachine = runTuringMachine' [] (clock (-1)) tm
